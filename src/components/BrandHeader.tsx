...
import { BRAND_CONSTANTS } from '../brand/brand';
...
export const BrandHeader: React.FC<Props> = ({ brand, openSettings, goHome }) => {
    return (
            ...
            <div className="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
                <div className="flex items-center gap-3 cursor-pointer" onClick={goHome}>
                    {brand.logo ? (
                        ...
                        <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center text-white font-bold font-secondary shadow-sm">
                            {BRAND_CONSTANTS.logoPlaceholder}
                        </div>
                    )}
                    <div>
                        <h1 className="font-primary font-bold text-lg text-primary leading-tight">{brand.name}</h1>
                        <p className="text-xs text-gray-500 font-secondary">{brand.tagline}</p>
                    </div>
                </div>
                <button 
